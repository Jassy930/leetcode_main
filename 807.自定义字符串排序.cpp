...
public:
    int maxIncreaseKeepingSkyline(vector<vector<int>>& grid) {
        vector<int> vv(grid.size());
        vector<int> hv(grid.at(0).size());
        for(int i=0; i<grid.size(); i++)
        {
            for(int k=0; k<grid.at(0).size(); k++)
            {
                vv.at(i) = max(vv.at(i), grid.at(i).at(k));
                hv.at(k) = max(hv.at(k), grid.at(i).at(k));
            }
        }
        int out = 0;
        for(int i=0; i<grid.size(); i++)
        {
            for(int k=0; k<grid.at(0).size(); k++)
            {
                out+= min(vv.at(i), hv.at(k)) - grid.at(i).at(k);
            }
        }
        return out;
    }
