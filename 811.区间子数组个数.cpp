...
class Solution
{
  public:
    vector<string> subdomainVisits(vector<string> &cpdomains)
    {
        ...
        {
            stringstream ss;
            int p = cpdomains.at(i).find(" ", 0);
            int num;
            ss << cpdomains.at(i).substr(0, p);
            ss >> num;
            for (int k = cpdomains.at(i).size() - 1; k >= 0; k--)
            {
                if (cpdomains.at(i).at(k) == '.' || cpdomains.at(i).at(k) == ' ')
                {
                    map<string, int>::iterator ii;
                    string s = cpdomains.at(i).substr(k + 1, cpdomains.at(i).size() - k);
                    // ii = timesmap.find(s);
                    // if (ii != timesmap.end())
                    //     timesmap[s] = num;
                    // else
        ...
        map<string, int>::iterator ii;
        for (ii = timesmap.begin(); ii != timesmap.end(); ii++)
        {
            stringstream ss;
            ss << ii->second << " " << ii->first;
